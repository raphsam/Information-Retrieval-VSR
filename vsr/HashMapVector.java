...
public class HashMapVector {
  ...
  // Used when adding occurences to the HashMapBigramVector
  public double incrementForBigram(String token, double amount) {
    Weight weight = hashMap.get(token);
    if (weight == null) {
      // If there is no current Weight for this token, create one
      weight = new Weight();
  ...
  // Used when adding occurences to the HashMapBigramVector
   public double incrementForBigram(String token) {
    return incrementForBigram(token, 1.0);
  }
