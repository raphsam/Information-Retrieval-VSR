...
public abstract class Document {
  ...
  public HashMapVector hashMapVector() {
	...
	Map<String, TokenInfo> bigramHash = InvertedPhraseIndex.getBigramHash();
    // Process each token in the document and add it to the vector
    while (hasMoreTokens()) {
      String token = nextToken();
	  String bigramToken;
	  // Check if it is a possible bigram
	  if (nextToken != null) {
		  bigramToken = (token + " " + nextToken);
		  // checks if bigram exists in bigramHash
		  if (bigramHash != null && (bigramHash.get(bigramToken) != null)) {
			  vector.increment(bigramToken);
			  // Since it tokenized the bigram, skip the next single word 
			  if (hasMoreTokens())
				  token = nextToken();
		  }
		  else 
  ...
  // Used in the first pass to find all the possible bigrams
  public HashMapVector hashMapBigramVector() {
    ...
    while (hasMoreTokens()) {
      String token = nextToken();
	  // add on the next token if it exits to create a bigram
	  if (nextToken == null)
		  return vector;
	  else { 
		token = (token + " " + nextToken);
		vector.incrementForBigram(token);
	  }
    }
    return vector;
  }
